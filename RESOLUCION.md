...
Para lograr paralelismo en Python se optó por *multiprocessing* en lugar de *multithreading*. La razón es el GIL (_Global Interpreter Lock_) de Python que se menciona en la consigna. Aunque con threads hay concurrencia (el GIL se libera durante operaciones de I/O), el procesamiento de cada batch —parseo, construcción de objetos, escritura al archivo— es código Python puro que el GIL serializa, permitiendo que solo un hilo avance a la vez. Con *multiprocessing*, cada proceso tiene su propio intérprete y su propio GIL, por lo que el trabajo de distintas agencias corre en paralelo en núcleos de CPU independientes.

El diseño mantiene la estructura del ejercicio anterior, donde el proceso principal acepta conexiones de forma secuencial y delega cada una a un proceso hijo. Una vez que el hijo es creado, el padre cierra su copia del socket y continúa aceptando la siguiente conexión. Cada proceso hijo maneja el ciclo de vida completo de su cliente, recibiendo todos los batches, almacenando las apuestas y esperando la señal de finalización.

Para coordinar la lotería entre procesos independientes se utilizan primitivas compartidas provistas por un `Manager`: una barrera que bloquea a cada proceso hasta que todos hayan recibido la señal de finalización de su agencia, un `Lock` y un `Event` para garantizar que la lotería se ejecute exactamente una vez, y un diccionario compartido donde se almacenan los ganadores agrupados por agencia. El proceso que llega último a la barrera adquiere el lock, verifica que la lotería aún no se ejecutó, la corre y setea el Event. Los demás esperan en ese Event antes de consultar el diccionario. De esta forma, una vez que todos los procesos superaron la barrera, cada uno lee sus propios ganadores del diccionario compartido y los envía a su cliente directamente, sin necesidad de que el proceso principal intervenga en el envío de resultados.
